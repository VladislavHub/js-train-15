...
function calculateCircleArea(radius) {
  ...
  if(typeof radius !== 'number'){
    console.log('Помилка. Значення повинно бути числом.');
    return null
  }
...
function findMinMax(numbers) {
  ...
  if(!Array.isArray(numbers)){
    console.log('Помилка. Значення повинно бути масивом.');
    return null
  }
...
function compareAndRound(num1, num2) {
  ...
  let minMax = Math.max(num1, num2)
  
  let result = Math.round(minMax)
...
function estimateInvestment(principal, interestRate, years) {
  ...
  let investmentAmount = principal * Math.pow(1 + interestRate, years);

  let roundedInvestment = Math.round(investmentAmount);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  return totalPriceRounded > maxPriceRounded;
}
